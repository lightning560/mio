...
// Component starts an Mio server for message consuming.
...
func (cmp *Component) Serve() error {
	switch cmp.mode {
...
func (cmp *Component) Healthz() bool {
	// todo 补齐
	return true
}
...
func (cmp *Component) launchOnConsumerGroupStart() error {
	...
	case originErr = <-handlerExit:
		if originErr != nil {
			cmp.logger.Error("terminating ConsumerGroup because an error", log.FieldErr(originErr))
		} else {
	...
	case <-cmp.ServerCtx.Done():
		...
		cmp.logger.Error("terminating ConsumerGroup because a context error", log.FieldErr(originErr))

		err := <-handlerExit
		if err != nil {
			cmp.logger.Error("terminating ConsumerGroup because an error", log.FieldErr(err))
		} else {
...
func (cmp *Component) launchOnConsumerStart() error {
	...
	case originErr = <-handlerExit:
		if originErr != nil {
			cmp.logger.Error("terminating ConsumerServer because an error", log.FieldErr(originErr))
		} else {
	...
	case <-cmp.ServerCtx.Done():
		originErr = cmp.ServerCtx.Err()
		cmp.logger.Error("terminating ConsumerServer because a context error", log.FieldErr(originErr))

		err := <-handlerExit
		if err != nil {
			cmp.logger.Error("terminating ConsumerServer because an error", log.FieldErr(err))
		} else {
			cmp.logger.Info("message handler exited without any error")
		}
	}
...
func (cmp *Component) launchOnConsumerEachMessage() error {
	...
	go func() {
		for {
			...
			if err != nil {
				...
				cmp.logger.Error("encountered an error while fetching message", log.FieldErr(err))
		...
		HANDLER:
			...
			metric.ClientHandleHistogram.WithLabelValues("kafka", compNameTopic, "HANDLER", brokers).Observe(time.Since(now).Seconds())
			if err != nil {
				metric.ClientHandleCounter.Inc("kafka", compNameTopic, "HANDLER", brokers, "Error")
			} else {
				metric.ClientHandleCounter.Inc("kafka", compNameTopic, "HANDLER", brokers, "OK")
			}

			if err != nil {
				cmp.logger.Error("encountered an error while handling message", log.FieldErr(err))
				cmp.consumptionErrors <- err
		...
		COMMIT:
			...
			// Record the redis time-consuming
			metric.ClientHandleHistogram.WithLabelValues("kafka", compNameTopic, "COMMIT", brokers).Observe(time.Since(now).Seconds())
			if err != nil {
				metric.ClientHandleCounter.Inc("kafka", compNameTopic, "COMMIT", brokers, "Error")
			} else {
				metric.ClientHandleCounter.Inc("kafka", compNameTopic, "COMMIT", brokers, "OK")
			}

			if err != nil {
				cmp.consumptionErrors <- err
				cmp.logger.Error("encountered an error while committing message", log.FieldErr(err))
	...
	case <-cmp.ServerCtx.Done():
		...
		cmp.logger.Error("terminating consumer because a context error", log.FieldErr(rootErr))
	...
	case originErr := <-unrecoverableError:
		...
		cmp.logger.Fatal("stopping server because of an unrecoverable error", log.FieldErr(originErr))
		cmp.Stop()
...
func (cmp *Component) closeConsumer(consumer *ekafka.Consumer) error {
	if err := consumer.Close(); err != nil {
		cmp.logger.Fatal("failed to close Consumer", log.FieldErr(err))
		return err
	}
...
func (cmp *Component) closeConsumerGroup(consumerGroup *ekafka.ConsumerGroup) error {
	if err := consumerGroup.Close(); err != nil {
		cmp.logger.Fatal("failed to close ConsumerGroup", log.FieldErr(err))
		return err
	}
...
func NewConsumerServerComponent(name string, config *config, ekafkaComponent *ekafka.Component, logger *log.Logger) *Component {
