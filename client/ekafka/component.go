...
func (cmp *Component) Producer(name string) *Producer {
	...
	if !ok {
		cmp.logger.Panic("producer config not exists", log.String("name", name))
	}
...
func (cmp *Component) Consumer(name string) *Consumer {
	...
	if !ok {
		cmp.consumerMu.Unlock()
		cmp.logger.Panic("consumer config not exists", log.String("name", name))
	}
...
func (cmp *Component) ConsumerGroup(name string) *ConsumerGroup {
	...
	if !ok {
		cmp.consumerGroupMu.Unlock()
		cmp.logger.Panic("consumerGroup config not exists", log.String("name", name))
	}
	...
	if err != nil {
		cmp.logger.Panic("create ConsumerGroup failed", log.FieldErr(err))
	}
