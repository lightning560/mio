		...
		for {
			for n := range rch {
				...
				if err := n.Err(); err != nil {
					log.Error(errors.MsgWatchRequestErr, log.FieldErrKind(errors.ErrKindRegisterErr), log.FieldErr(err), log.FieldAddr(prefix))
					continue
				}
				for _, ev := range n.Events {
					...
					case w.eventChan <- ev:
					default:
						log.Error("watch etcd with prefix", log.Any("err", "block event chan, drop event message"))
					}
				}
			}
			ctx, cancel := context.WithCancel(context.Background())
