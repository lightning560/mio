...
func newETCDRegistry(config *Config) (*etcdv3Registry, error) {
	...
	config.logger = config.logger.With(log.FieldMod(errors.ModRegistryETCD), log.FieldAddrAny(config.Config.Endpoints))
...
func (reg *etcdv3Registry) ListServices(ctx context.Context, name string, scheme string) (services []*server.ServiceInfo, err error) {
	...
	if getErr != nil {
		reg.logger.Error(errors.MsgWatchRequestErr, log.FieldErrKind(errors.ErrKindRequestErr), log.FieldErr(getErr), log.FieldAddr(target))
		return nil, getErr
	}
	...
	for _, kv := range getResp.Kvs {
		...
		if err := json.Unmarshal(kv.Value, &service); err != nil {
			reg.logger.Warnf("invalid service", log.FieldErr(err))
			continue
		}
...
func (reg *etcdv3Registry) Close() error {
	...
	reg.kvs.Range(func(k, v interface{}) bool {
		...
		go func(k interface{}) {
			...
			if err != nil {
				reg.logger.Error("unregister service", log.FieldErrKind(errors.ErrKindRequestErr), log.FieldErr(err), log.FieldErr(err), log.FieldKeyAny(k), log.FieldValueAny(v))
			} else {
				reg.logger.Info("unregister service", log.FieldKeyAny(k), log.FieldValueAny(v))
			}
			cancel()
		}(k)
		return true
...
func (reg *etcdv3Registry) registerMetric(ctx context.Context, info *server.ServiceInfo) error {
	...
	if err != nil {
		reg.logger.Error("register service", log.FieldErrKind(errors.ErrKindRegisterErr), log.FieldErr(err), log.FieldKeyAny(key), log.FieldValueAny(info))
		return err
	}

	reg.logger.Info("register service", log.FieldKeyAny(key), log.FieldValueAny(val))
...
func deleteAddrList(al *registry.Endpoints, prefix, scheme string, kvs ...*mvccpb.KeyValue) {
	for _, kv := range kvs {
		...
		if strings.HasPrefix(addr, "providers/"+scheme) {
			...
			if err != nil {
				log.Error("parse uri", log.FieldErrKind(errors.ErrKindUriErr), log.FieldErr(err), log.FieldKey(string(kv.Key)))
				continue
			}
...
/*
key: /mio/main/configurator/grpc:///routes/1
val:
{
	"upstream": { // 客户端配置
		"nodes": { // 按照node负载均衡
...
key: /mio/main/configurator/grpc://127.0.0.1/routes/2
val:
{
	"upstream": { // 客户端配置
		"nodes": { // 按照node负载均衡
...
key: /mio/main/configurator/grpc:///consumers/client-demo
val:
{
