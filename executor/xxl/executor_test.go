...
func Test_StdNewExecutor(t *testing.T) {
	...
	[xxl]
		[xxl.job]
			[xxl.job.executor]
				appname = "mio-xxl-job-demo"  # 执行器名称
				port = "59001"                    # 开启执行器的服务端口
				access_token = "mio-token"    # xxl-job需要的token信息
	...
	tests := []struct {
	...
	}{
		{
			...
			want: Options{
				...
				AccessToken:   "mio-token",
				ExecutorIp:    ipv4.LocalIP(),
				ExecutorPort:  "59001",
				RegistryKey:   "mio-xxl-job-demo",
				RegistryGroup: "EXECUTOR",
				LogDir:        constants.BasePath + "mio-xxl-job-demo" + "/jobhandler/",
				Switch:        true,
				Debug:         false,
			},
		},
		{
			...
			opts: []Option{ExecutorHost("127.0.0.1"), ExecutorPort("7894"), AccessToken("test-token"),
				Debug(), RegistryKey("mio-demo"), RegistryGroup("CONTAINS"), Switch(false),
			...
			want: Options{
				...
				ExecutorPort:  "7894",
				RegistryKey:   "mio-demo",
				RegistryGroup: "CONTAINS",
				LogDir:        constants.BasePath + "mio-demo/jobhandler/",
				Switch:        false,
				Debug:         true,
			},
		},
...
func Test_handlerWithAuth(t *testing.T) {
	...
	req_succuss.Header.Add("XXL-JOB-ACCESS-TOKEN", "mio-token")
	req_fail := &http.Request{
		Header: http.Header{},
	}
	req_fail.Header.Add("XXL-JOB-ACCESS-TOKEN", "mio-token_fail")
