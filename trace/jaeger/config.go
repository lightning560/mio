...
func DefaultConfig() *Config {
	return &Config{
		...
		AgentHost:          xenv.EnvOrStr("OTEL_EXPORTER_JAEGER_AGENT_HOST", "localhost"),
		AgentPort:          xenv.EnvOrStr("OTEL_EXPORTER_JAEGER_AGENT_PORT", "6831"),
		CollectorEndpoint:  xenv.EnvOrStr("OTEL_EXPORTER_JAEGER_ENDPOINT", "http://localhost:14268/api/traces"),
		CollectorUser:      xenv.EnvOrStr("OTEL_EXPORTER_JAEGER_USER", ""),
		CollectorPassword:  xenv.EnvOrStr("OTEL_EXPORTER_JAEGER_PASSWORD", ""),
...
func (config *Config) Build(ops ...Option) trace.TracerProvider {
	...
	default:
		log.Panic("jaeger type error", log.FieldName(config.JaegerEndpointType))
	}
	...
	options := []tracesdk.TracerProviderOption{
		...
		// Record information about this env in an Resource.
