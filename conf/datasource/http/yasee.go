...
func (y *yaseeDataSource) watch() {
	for {
		...
		// client get err
		if err != nil {
			log.Error("yaseeDataSource", log.String("listenConfig curl err", err.Error()))
			time.Sleep(time.Second * 1)
			continue
		}
		if resp.StatusCode() != 200 {
			log.Error("yaseeDataSource", log.String("listenConfig status err", resp.Status()))
			time.Sleep(time.Second * 1)
			continue
		}
		...
		if yaseeRes.Code != 200 {
			log.Info("yaseeDataSource", log.Int64("code", int64(yaseeRes.Code)))
			time.Sleep(time.Second * 1)
			continue
		}
...
func (y *yaseeDataSource) updateConfig(data string, version int64) {
	...
	case y.changed <- struct{}{}:
		...
		log.Info("yaseeDataSource", log.String("change", data))
	default:
	}
}
